...
class EnhancedRAGPipeline:
    ...
    def create_user_profile_json(self, mood: str, budget: str, time: str, 
        ...
        user_profile = {
            ...
            "location": {
                ...
                "search_radius_km": 2.0  # Start with 2km
            },
            "current_location": {
    ...
    def expand_search_radius(self, user_profile: Dict, current_results: int) -> Dict:
        ...
        if current_results < 3:
            if current_radius < 5:
                user_profile["location"]["search_radius_km"] = 5.0
...
1. "exclude_category:[category]" - If user doesn't want a specific type of place
   Examples: "i dont want temples", "not interested in cafes", "no bars please"

2. "category:[category]" - If user wants to see a specific type of place (HANDLE TYPOS!)
   Examples: "show me cafes", "parks", "shoe park", "i want restaurants", "find parks", "any malls?", "templs"

3. "i've visited [place_name]" - If user mentions they've been to a specific place
   Examples: "ive been to legacy brewery", "visited subko", "already went to that cafe"

4. "regenerate_all" - If user wants completely different recommendations
   Examples: "other options", "change the plan", "different places", "show something else"
...
Be smart about typos and context. If the message seems like a category request (even with typos), respond with category:[corrected_category].
